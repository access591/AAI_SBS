...
public class DateValidation {
	...
	public String convertDateFormat(String date) throws InvalidDataException {
		...
		if(checkDelimeters(date.toCharArray())==true){
			if(date.indexOf("'")!=-1){
				String rmdelimiter="\'";
				date=StringUtility.replace(date.trim().toCharArray(),rmdelimiter.toCharArray(),"").toString();
			}else {
				date=StringUtility.replaces(date.trim().toCharArray(),rmdelimiters,"").toString();
			}
		}
		...
		if(newDateFormat.indexOf("/")!=-1){
			...
			if(!newDateFormat.equals("")){
				finalDate=commonUtil.converDBToAppFormat(newDateFormat,"MM/dd/yyyy","dd-MMM-yyyy");
			}else{
	...
	private  String checkDayFormat(String dateFormat){
		...
		if(index!=0){
			...
			finalday=StringUtility.replace(tempDayVal.toCharArray(),day.toCharArray(),replacedDay).toString();
			finalday=finalday+dateFormat.substring(index+1,dateFormat.length());
	...
	private  String checkMonthYearFormat(String dateFormat){
		...
		if(strtIndex!=-1 && endIndex!=-1){
			log.info("======checkMonthYearFormat======"+strtIndex+"endIndex"+endIndex);
			if(strtIndex==endIndex){
				tempDateFrmt=dateFormat.substring(strtIndex+1,dateFormat.length());
				...
				log.info("======checkMonthYearFormat======(tempMonth)"+tempMonth);
				log.info("======checkMonthYearFormat======(tempYear)"+tempYear);
					...
					if(mnthFlag==true){
						...
						if(year.length() > 4){
							...
							for(int i=1;i<year.length();i++){
								...
								if(chkflag!=true){
									if(Integer.parseInt(chkYear)==19 || Integer.parseInt(chkYear)==20){
										chkflag=true;
										year=year.substring(strtInd,year.length());
									}else{
										strtInd=i+odd;
										endInd=i+even;
										chkYear=year.substring(strtInd,endInd);
									}
								}
						...
						fnalDtFrmt=StringUtility.replace(dateFormat.toCharArray(),oldDateVal.toCharArray(),replacedVal).toString();
						log.info("======checkMonthYearFormat======(fnalDtFrmt)"+fnalDtFrmt);
					}else{
						/*
				...
				if(!tempMonth.equals(oldDateVal)){
					fnalDtFrmt=StringUtility.replace(dateFormat.toCharArray(),oldDateVal.toCharArray(),tempMonth).toString();
				}else{
	...
	private  String chkAlphaNumber(String value){
		...
		if(!month.equals(value)){
			finalValue=StringUtility.replace(value.toCharArray(),findValue.toCharArray(),month).toString();
		}else{
			finalValue=value;
		}
	...
	private  String validateAlphaNumber(char[] frmtDt){
		...
		if(!validDt.equals("")){
			...
			if(validDt!=null && !validDt.equals("")){
				...
				if(validDate.indexOf(".")!=-1 && (validDate.indexOf(" ")!=-1)){
					String rmdelimiter=".";
					validDate=StringUtility.replace(validDate.trim().toCharArray(),rmdelimiter.toCharArray(),"").toString().trim();
				}
			}
		}else{
			validDate=finalBuff.toString().trim();
