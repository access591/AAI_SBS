...
evt.returnValue = false;
   ...
   function editCpf(prevemol,oldemoluments,oldepf,oldvpf,oldprincipal,oldintrest,pensionno,monthyear,airportcode,region,emolumentstextboxno,epftextboxno,vpftextboxno,principaltextboxno,intresttextboxno,remarkstypetextboxno,remarkstextboxno,editid,userId){
	 ...
	 document.getElementsByName(remarkstypetextboxno)[0].style.background='#FFFFCC'; 
	  buttonName=document.getElementsByName(editid)[0].value;	   
	 ...
	 if(remarks==''|| Math.round(remarks)=='0'){
	 alert("Please select remarks and remarks should be greaterthan 0");
	 ...
	 document.getElementsByName(remarkstypetextboxno)[0].focus();
	   return false;
	 	 	  ...
	 	 	  if(remarksType == 'NCP'){	 
	   if(parseFloat(remarks)>31){
	    alert("NCP Days cannot be  more than 31 days. for the PFID "+pensionno+"");
	    document.getElementsByName(remarkstextboxno)[0].focus();
	   return false;
	   }
	 if(parseFloat(emoluments)>parseFloat(prevemol)){	
	  alert("For the RemarkType NCP Days Emoluments cannot be greaterthan the previous Emoluments  for the PFID "+pensionno+"");
	 document.getElementsByName(remarkstypetextboxno)[0].focus();
	   return false;
	 }
	 } 
	 if(remarksType == 'prevNCP'){
	  if(parseFloat(remarks)>31){
	    alert("Prev NCP Days cannot be  more than 31 days. for the PFID "+pensionno+"");
	    document.getElementsByName(remarkstextboxno)[0].focus();
	   return false;
	   }
	 if(parseFloat(emoluments)<parseFloat(prevemol)){	
	  alert("For the RemarkType Prev NCP Days Emoluments cannot be lessthan the previous Emoluments  for the PFID "+pensionno+"");
	 document.getElementsByName(remarkstypetextboxno)[0].focus();
	   return false;
	 }
	 }
	 ...
	 if(remarksType == 'paystop'){
	  if(parseFloat(emoluments)>0 || parseFloat(emoluments)<0){	
	  alert("For the RemarkType Pay Stop Emoluments cannot be greaterthan 0  for the PFID "+pensionno+"");
	 document.getElementsByName(remarkstypetextboxno)[0].focus();
	   return false;
	 }
	 } 
	   if( remarksType == 'NonWageArrear'){	
	    if(parseFloat(emoluments)<parseFloat(prevemol)){	
	  alert("For the  Non Wage Arrears Emoluments cannot be lessthan the previous Emoluments  for the PFID "+pensionno+"");
	 document.getElementsByName(remarkstypetextboxno)[0].focus();
	   return false;
	 }	
	 } 
	  ...
	  if(parseFloat(emoluments)>parseFloat(prevemol)){	
	  alert("For the RemarkType Prev Non Wage Arrear Emoluments cannot be greaterthan the Emoluments  for the PFID "+pensionno+"");
	 document.getElementsByName(remarkstypetextboxno)[0].focus();
	   return false;
	 }
	 } 
	  ...
	  if(parseFloat(emoluments)>parseFloat(prevemol)){	
	  alert("For the RemarkType Prev Wage Arrear Emoluments cannot be greaterthan the Emoluments  for the PFID "+pensionno+"");
	 document.getElementsByName(remarkstypetextboxno)[0].focus();
	   return false;
	 }
	 }
  ...
  function insertCpfData(monthyear){
  ...
  var url="<%=basePath%>validatefinance?method=insertCpfDataDirectly&userid="+username+"&monthyear="+monthyear; 
		document.forms[0].action=url;
		document.forms[0].method="post";
		document.forms[0].submit();	
  }
function deleteCpfData(monthyear){
   var username='<%=username%>';
  var agree=confirm("Are you sure you want to delete?");       
  if (agree){
  var url="<%=basePath%>validatefinance?method=deleteimportsheetdirect&monthyear="+monthyear+"&userid="+username;
		document.forms[0].action=url;
		document.forms[0].method="post";
		document.forms[0].submit();
		}
...
<tr>
	...
	<td class="Data" width="4%" align="right"> <%=validateInfo.getPensionNumber()%></td> 	
	...
	<td class="Data" width="4%" align="right"><select disabled="disabled" onchange="changere('<%=validateInfo.getEmoluments()%>','<%=validateInfo.getEmpPFStatuary()%>','<%=validateInfo.getEmpVPF()%>','<%=validateInfo.getEmpAdvRecPrincipal()%>','<%=validateInfo.getEmpAdvRecInterest()%>','dispEmoluments<%=i%>','dispEPF<%=i%>','dispVPF<%=i%>','dispprincipal<%=i%>','dispintrest<%=i%>','dispremarkstype<%=i%>');" style="width:130px" 
								...
								<option value="paystop" <%if(validateInfo.getIpAddress().trim().equals("paystop")){ out.println("selected");}%>>Pay Stop</option>
								<option value="NCP" <%if(validateInfo.getIpAddress().trim().equals("NCP")){ out.println("selected");}%>>NCP Days</option>
								<option value="prevNCP" <%if(validateInfo.getIpAddress().trim().equals("prevNCP")){ out.println("selected");}%>>Prev NCP Days</option>
								...
								<option value="PWarrear" <%if(validateInfo.getIpAddress().trim().equals("PWarrear")){ out.println("selected");}%>>Prev Pay Rev Arr(01-01-2007)</option>
								<option value="WageArrear" <%if(validateInfo.getIpAddress().trim().equals("WageArrear")){ out.println("selected");}%>>Pay Rev Arr(01-01-2007)</option>
 </select></td>	
...
<%}%>
   ...
   <td class="btn" align="right"><input type="button" name="update" id="update" value="Finalize" onclick="insertCpfData('<%=validateInfo.getMonthYear()%>')"/> </td>
   <td>&nbsp;</td>
   <td>&nbsp;</td>
    <td class="btn" align="left"><input type="button" name="rollback" id="rollback" value="Delete" onclick="deleteCpfData('<%=validateInfo.getMonthYear()%>')"/></td>
  <td>&nbsp;</td>
  <td>&nbsp;</td>
   </tr>   
   </table>	
