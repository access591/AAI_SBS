...
evt.returnValue = false;
  ...
  var url="<%=basePath%>validatefinance?method=insertCpfData&monthyear="+monthyear+"&station="+airportcode+"&regn="+region;
...
<tr>
	...
	<td class="Data" width="4%" align="right"><select disabled="disabled" onchange="changere('<%=validateInfo.getEmoluments()%>','<%=validateInfo.getEmpPFStatuary()%>','<%=validateInfo.getEmpVPF()%>','<%=validateInfo.getEmpAdvRecPrincipal()%>','<%=validateInfo.getEmpAdvRecInterest()%>','dispEmoluments<%=i%>','dispEPF<%=i%>','dispVPF<%=i%>','dispprincipal<%=i%>','dispintrest<%=i%>','dispremarkstype<%=i%>');" style="width:130px" 
								...
								<option value="PWarrear" <%if(validateInfo.getIpAddress().trim().equals("PWarrear")){ out.println("selected");}%>>Prev Pay Rev Arrear(01-01-2007)</option>
								<option value="WageArrear" <%if(validateInfo.getIpAddress().trim().equals("WageArrear")){ out.println("selected");}%>>Pay Rev Arrear(01-01-2007)</option>
 </select></td>		
