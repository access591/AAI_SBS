...
var arrtemp = new Array();
  ...
  return false;
   ...
   var username='<%=username%>';  
  var url="<%=basePath%>validatefinance?method=loadimportsheetdirectly&userid="+username;
...
<table width="100%" border="0" cellspacing="0" cellpadding="0">   
 ...
 <tr><td>&nbsp;</td></tr>  
  ...
  <tr>
    ...
    <td colspan="8">  
 <%
           if (request.getAttribute("transferlist") != null) {
           ...
           String userId = session.getAttribute("userid").toString();
               ...
               ArrayList rauList= new ArrayList();    
                ...
                int l=0; 
                 empinfo = (ArrayList) request.getAttribute("transferlist");
                  
                 for (int k = 0; k < empinfo.size(); k++) {
                 rauList = (ArrayList) empinfo.get(k);          
                   if(rauList.size()>0){
				empList = (ArrayList)rauList.get(0);
				cntList = (ArrayList)rauList.get(1);
				}             
			...
			<tr>
                ...
                for (int i = 0; i < empList.size(); i++) {
                    count++; 
                    l++;                  
                     beans = (EmployeePersonalInfo) empList.get(i);
                   ...
                   String salaryMonth=beans.getMonthyear();
                    String airportcode=beans.getAirportCode();
                       temp += PENSIONNO + ",";   
                       System.out.println(beans.getRefPensionNumber());
                        editarray +=beans.getRefPensionNumber()+",";
                 	System.out.println("editarray"+editarray);
                   %>
						<tr> 
    ...
    <td class="Data" width="4%" align="right">
       <select disabled="disabled"  style="width:110px" name="disptransfertype<%=l%>" id="disptransfertype<%=l%>">
 ...
 <%}%>
   ...
   <td class="Data"><input type="button" name="edit<%=l%>"  width="2%" style = "cursor:hand;" 
					value="E" onclick="editCpf('<%=DATEOFBIRTH%>','<%=DATEOFJOINING%>','<%=PENSIONNO%>','<%=salaryMonth%>','<%=airportcode%>','<%=REGION%>','disptransfertype<%=l%>','edit<%=l%>','<%=userId%>')" /></td>
 </tr>  	
<%}%>
	</table>
	<%
		System.out.println("===========cntinfo======="+cntList.size());
		if(cntList.size()>0){
		
		
		for(int j=0;j<cntList.size();j++){		
		  EmployeePersonalInfo cntInfo = (EmployeePersonalInfo) cntList.get(j);
			%>
			<tr>
			<td>&nbsp;</td>
			</tr>       
   ...
   <tr><td>&nbsp;</td></tr>
    ...
    <td >&nbsp;</td>
     ...
     <table align="center" width="100%" cellpadding="1"  cellspacing="1" border="0">
      <tr>	
       <td class="btn" align="right"><input type="button" name="update" id="update" value="Finalize" onclick="insertCpfData()"/></td>	
      <td width="1%">&nbsp;</td>
     <td class="btn" align="left"><input type="button" name="rollback" id="rollback" value="Delete" onclick="deleteCpfData('<%=beans.getMonthyear()%>')"/></td>
    </tr>
   </table>  
  <tr><td>&nbsp;</td></tr>
  <table align="center" width="100%" cellpadding="1"  cellspacing="1" border="0">
