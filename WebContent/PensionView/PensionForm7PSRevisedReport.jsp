...
<html>
	...
	%>
		...
		dispYear=request.getAttribute("dspYear").toString();
						...
						dataList=(ArrayList)request.getAttribute("form8List");
							...
							arrearRevisedFlag=personalInfo.getArrearRevisedFlag();
							//System.out.println("arrearRevisedFlag "+arrearRevisedFlag);
  ...
  <tr>
    ...
    <%
    	...
    	//System.out.println("getTransArrearFlag=============="+cardInfo.getTransArrearFlag()+"arrearRevisedFlag"+arrearRevisedFlag);
    	dueTotalEmoluments=Math.round(Double.parseDouble(cardInfo.getEmoluments()))+Math.round(Double.parseDouble(cardInfo.getDueemoluments()));
    ...
    %>
      ...
      <tr>
        ...
        <%if(cardInfo.getTransArrearFlag().equals("Y") ){%>
         <td class="Data" width="30%" align="right"><%=-arrearEmoluemntsAmount%></td>
        <%}else{%>
        ...
        <%if(cardInfo.getTransArrearFlag().equals("Y") ){%>
        <td class="Data" width="23%" align="right"><%=-arrearContriAmount%></td>
          <%}else{%>
         <td class="Data" width="30%" align="right"><%=Math.round(Double.parseDouble(cardInfo.getDuepensionamount()))%></td>
        <%}%>
      	...
      	if(monthYear.equals("Apr-1995")){
  		 ...
  		 totalPension=totalPension+Math.round(Double.parseDouble(cardInfo.getPensionContribution()));
  		  if(arrearflags==true){
  		  	...
  		  	dueEmoluments=dueEmoluments-arrearEmoluemntsAmount;
  		  	duePension=duePension-arrearContriAmount;
  		  	arrearflags=false;
      	   }
      	   System.out.println("totalEmoluments"+totalEmoluments+"dueTotalEmoluments"+dueTotalEmoluments+"totalPension"+totalPension+"dueTotalPension"+dueTotalPension);
 ...
 <tr  bordercolor="white">
 <td colspan="5">Certified that the difference between the total of the contribution shown under col.(7) of the above table and that
  ...
  <%if(size-1!=i){%>
	...
	<%}%>	
<%totalEmoluments=0.0;totalPension=0.0;dueEmoluments=0.0;duePension=0.0;dueGrandTotalEml=0.0;dueGrandTotalPension=0.0;seperationFlag=false;remarks="";}%>
