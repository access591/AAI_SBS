...
<html>
	...
	function redirectPageNav(navButton,index,totalValue){      
   	  	...
   	  	document.forms[0].action="<%=basePath%>PensionLogin?method=groupNavigation&navButton="+navButton+"&strtindx="+index+"&total="+totalValue;
  ...
  function testSS(){	
   		document.forms[0].action="<%=basePath%>PensionLogin?method=searchgroup";
		document.forms[0].method="post";
		document.forms[0].submit();
  } 
  ...
  {
	 document.forms[0].action="<%=basePath%>/PensionView/UserGroupSearch.jsp";    	
	 document.forms[0].method="post";	
	 document.forms[0].submit();         	   
  }   
 ...
 function editUnitMaster(obj1,obj2){	
	    ...
	    if(answer){
		document.forms[0].action="<%=basePath%>PensionLogin?method=GroupUserEdit&groupid="+groupid+"&groupname="+groupname;		
		document.forms[0].method="post";
		document.forms[0].submit();
	}
	}
			   ...
			   if (request.getAttribute("searchBean") != null) {
				...
				<tr >
					...
					<table align="center" cellpadding=2 class="tbborder" cellspacing="0"  border="0">
							...
							int groupid=0;
							String username="",groupname="",pensionno="";
					...
					%>
 						...
 						<%if(session.getAttribute("userid").equals("NAGARAJ")){%>						
							  <td> 
									<input type="checkbox" name="unitcd" value="<%=groupid%>" onclick="javascript:editUnitMaster('<%=groupid%>','<%=groupname%>')" />
								</td>	
                          <%}else{%>   
                           <td>
								<input type="checkbox" name="unitcd" value="<%=groupid%>"	onclick="javascript:alert('Permissions Denied to Edit the record')"/> 
								</td>
                          <%} %>   			
