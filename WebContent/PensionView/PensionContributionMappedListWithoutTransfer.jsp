...
<html>
	...
	function testSS(){
  	  ...
  	  if(document.forms[0].empName.value=="" && document.forms[0].pfId.value=="" && document.forms[0].uanNo.value==""){
   		...
   		var url ="<%=basePath%>search1?method=getEmployeeMappedList&region="+region1+"&empName="+empName+"&uanNo="+uanNo+"&transferType="+'<%=transferStatus%>'+"&pfId="+pfId+"&screen="+pcreport+"&adjPfidChkFlag="+'<%=adjPfidChkFlag%>'+"&statemntRevisdChkFlag="+'<%=statemntRevisdChkFlag%>'+"&frmName="+'<%=frmName%>'+"&pcReportChkFlag="+'<%=pcReportChkFlag%>';
   		//alert(url);
		...
		<form>
			<table align="center">
				<tr>
					...
					<td>
						<input type="text" size=15 name="empName" onkeyup="return limitlength(this, 20)">
						&nbsp;
					</td>
					<td class="label">
						UAN No:
				...
				<td class="label">
					...
					<td>
						<input type="text" size=8 name="pfId" onkeyup="return limitlength(this, 20)">
						&nbsp;
					</td>&nbsp;
