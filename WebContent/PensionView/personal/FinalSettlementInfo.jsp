...
<html>
	...
	function testSS(){
  	 ...
  	 if(document.forms[0].empName.value=="" && document.forms[0].pfId.value==""){
   		...
   		var transferStatus="";
   		var url ="<%=basePath%>search1?method=getEmployeeFilasettledateinfo&region="+region1+"&empName="+empName+"&transferType="+transferStatus+"&pfId="+pfId;
   		statusdisplay();       
	    document.forms[0].action=url;
	    document.forms[0].method="post";
		document.forms[0].submit();
	}	
	 function editPensionMaster(cpfaccno,region,pensionNumber,employeeName,desig,dateofbirth,dateofJoining,airportCode,finalsetdate,resetdate,intcalcdate,reintcalcdate,seperationDate,seperationReason,remarks,settlementClient){
	 var mappedInfo=cpfaccno+"@"+region+"@"+pensionNumber+"@"+employeeName+"@"+desig+"@"+dateofbirth+"@"+dateofJoining+"@"+pensionNumber+"@"+airportCode+"@"+finalsetdate+"@"+resetdate+"@"+intcalcdate+"@"+reintcalcdate+"@"+seperationDate+"@"+seperationReason+"@"+remarks+"@"+settlementClient;
	// var url="<%=basePath%>PensionView/personal/EmployeeFinalsettlementDateEdit.jsp?mappedInfo="+mappedInfo;
	//alert("url"+url); 
	//document.forms[0].action=url;
	//document.forms[0].method="post";
	//document.forms[0].submit();
	var finalist='<%=finallist%>';
	var chkpfid='<%=chkpfid%>';
	 window.opener.test(cpfaccno,region,pensionNumber,employeeName,desig,dateofbirth,dateofJoining,airportCode,finalsetdate,resetdate,intcalcdate,reintcalcdate,seperationDate,seperationReason,remarks,settlementClient,finalist);	
	 window.close();	 
	}
	 </script>
	</head>
    ...
    <%if(!region.equals("AllRegions")) {%>
			...
			<td>		<input type="button" class="btn" value="Search" class="btn" onclick="testSS();">
				</td>
               </tr>
			...
			<tr>
				<td height="25%">
					<table align="center" width="97%" cellpadding=2 class="tbborder" cellspacing="0" border="0">
						...
						<%}%>
						<%int count = 0;
                String airportCode = "", employeeName = "", cpfacno = "", pensionNumber = "",desig="";
                String dateofBirth = "", empSerialNo = "", dateofJoining = "", pensionOption="";
                ...
                for (int i = 0; i < datalist.size(); i++) {
                    ...
                    %>
						...
						<tr>
							...
							<td>																																
								<input type="checkbox" name="cpfno" value="<%=cpfacno%>" onclick="editPensionMaster('<%=cpfacno%>','<%=region%>','<%=pensionNumber%>','<%=employeeName%>','<%=desig%>','<%=dateofBirth%>','<%=dateofJoining%>','<%=airportCode%>','<%=finalsetdate%>','<%=resetdate%>','<%=intcalcdate%>','<%=reintcalcdate%>','<%=seperationDate%>','<%=seperationReason%>','<%=remarks%>','<%=beans.getSettlementClient()%>');" />
							</td>
						</tr>
						<%}%>
						<%}%>
