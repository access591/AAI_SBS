...
<html>
	...
	function validate(){
	     document.forms[0].action='<%=basePath%>psearch?method=arrearsnav&pensioncontriadj=false';
	     document.forms[0].method="post";
		 document.forms[0].submit();
  }

	function getArrearData(pensionno,dateOfAnnuation,pensionoption,employeename){
		
		 document.forms[0].action="<%=basePath%>psearch?method=getArrearsTransactionData&pensionno="+pensionno+"&dateOfAnnuation="+dateOfAnnuation+"&wetheroption="+pensionoption+"&employeename="+employeename;
		 document.forms[0].method="post";
		 document.forms[0].submit();
	}
	</script>
 ...
 <%} %>
				<tr >
   				...
   				<td  class="Data" ><a href="#" title="Click the link to Enter Arrears" onClick="getArrearData('<%=beans.getPensionNumber()%>','<%=beans.getDateOfAnnuation()%>','<%=beans.getWetherOption()%>','<%=beans.getEmpName()%>')"><img src="./PensionView/images/editIcon.gif" border='0'  width='12' height='12' ></a>
									&nbsp;&nbsp;
								</td>
