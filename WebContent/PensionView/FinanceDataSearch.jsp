...
<script type="text/javascript"> 
	...
	window.status = comment; return true;
	 ...
	 function testSS(){
		 ...
		 if(document.forms[0].cpfaccno.value=="" && document.forms[0].employeeCode.value=="" && document.forms[0].pfid.value==""){
			 alert('Please Enter Cpf Accno or Employee Number or PF ID ');
			 document.forms[0].cpfaccno.focus();
			 return false;
			 }
			 if(document.forms[0].cpfaccno.value!="" && document.forms[0].region.selectedIndex=="0"){
				 alert("please select rhe region");
				 document.forms[0].region.focus();
				 return false;
			 }
			 if(document.forms[0].employeeCode.value!="" && document.forms[0].region.selectedIndex=="0"){
		...
		if(missingflag=='N'){
		  	path="<%=basePath%>validatefinance?method=getFinanceDetailEdit&cpfaccno="+cpfaccno+"&pensionno="+pensionno+"&airportCD="+unitCD+"&effectiveDate="+effectiveDt+"&employeeNM="+employeeName+"&employeeno="+employeeno+"&designation="+designation+"&region="+region;
		}else{
			  path="<%=basePath%>PensionView/AddFinancialDetail.jsp?effectiveDate="+effectiveDt;
		}
...
<table width="100%" border="0" align="center" cellpadding="0"
	...
	<tr>
		...
		<table align="center">
			<tr>
				...
				<td><input type="text" name="pfid"></td>
			</tr>
			<tr>
			...
			<tr>
				<td class="label">Missing Data:</td>
	...
	<tr>
		...
		<table align="center">
			<tr>
				<td colspan="3"></td>
				<td colspan="2" align="right"><!-- 	<input type="button" class="btn" alt="first" value="|<" name="First" disable=true onClick="javascript:redirectPageNav('|<','<%=index%>','<%=totalData%>')" <%=bottomGrid.getStatusFirst()%>>
				...
				<img src="./PensionView/images/printIcon.gif" alt="Report"
					onClick="callReport()"> <%}%>
				</td>
			</tr>
		</table>
	...
	<tr>
		...
		<table align="center" cellpadding=2 class="tbborder" cellspacing="0"
			...
			<tr class="tbheader">
				...
				<td class="tblabel">AirportCode</td>
				...
				<td class="tblabel">Finance Date&nbsp;&nbsp;&nbsp;&nbsp;</td>
				<!--<td class="tblabel">
									&nbsp;&nbsp;Emoluments &nbsp;&nbsp;&nbsp;&nbsp;
								</td>
								<td class="tblabel">
			...
			<tr>
				...
				<td class="Data"><%=beans.getCpfAccNo()%></td>
				<td class="Data"><%=beans.getEmployeeName()%> &nbsp;&nbsp;</td>
				<td class="Data"><%=beans.getEmployeeNewNo()%> &nbsp;&nbsp;</td>
				<td class="Data"><%=beans.getDesignation()%></td>
				<td class="Data"><%=beans.getMonthYear()%></td>
				<td><img alt="" src="<%=beans.getIcon()%>"
					onclick="javascript:submit_Pension('<%=beans.getMonthYear()%>','<%=beans.getEmployeeName()%>','<%=beans.getCpfAccNo()%>','<%=beans.getPensionNumber()%>','<%=beans.getEmployeeNewNo()%>','<%=beans.getDesignation()%>','<%=beans.getAirportCode()%>','<%=beans.getMissingFlag()%>','<%=beans.getRegion()%>')">
				</td>
			</tr>
			<%}%>
			...
			<%if (dataList.size() != 0) {%>
			<tr>
				<td><font color="red"><%=index%></font> &nbsp;Of&nbsp;&nbsp;<font
					color="red"><%=totalData%></font>&nbsp;Records</td>
			</tr>
			<%}}
			%>
