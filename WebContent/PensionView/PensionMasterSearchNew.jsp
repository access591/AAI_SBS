...
//	alert(sURL);
	...
	function editPensionMaster(obj,employeeName,employeeCode,region,airportCode,index,totalData){
	 //	alert(index);
		var cpfacno=obj;
	...
	function getAirports(){
	 ...
	 alert("index "+index);
	 region=document.forms[0].region.options[document.forms[0].region.selectedIndex].text;
	  alert("region "+region);
        document.forms[0].action="<%=basePath%>search1?method=getAirportsList";
		document.forms[0].method="post";
		document.forms[0].submit();
 ...
 function testSS(){
    ...
    var index=document.forms[0].region.selectedIndex;
       if(document.forms[0].region.selectedIndex<1)
    	...
    	document.forms[0].action="<%=basePath%>search1?method=searchPensionMaster"
		document.forms[0].method="post";
		document.forms[0].submit();
   		 }
	...
	<body class="BodyBackground" onload="frmLoad();">
		<form name="test" action="" method="get">
				...
				<tr>
					<td height="15%">
						<table align="center">
						    ...
						    %>	
								<td>
									<SELECT NAME="region" style="width:130px" onchange="getAirports()">
										<option value="">
											[Select One]
										</option>
										<%int j = 0;
					...
					<%} else {%>
						...
						%>					
						<td>
						<input type="text" name="region" value="<%=session.getAttribute("region")%>" readonly="true">
						</td>
						<%
						}
						%>							
			...
			if (request.getAttribute("searchBean") != null) {
					...
					<td height="25%">
						<table align="center" width="97%" cellpadding=2 class="tbborder" cellspacing="0" border="0">
							<tr class="tbheader">
								...
								<td>
									<a href="<%=basePath%>search1?method=delete"> <img src="./PensionView/images/cross.png" alt="delete" onclick="deleteMultipule();" /></a>
								</td>
				...
				String dateofBirth = "", pensionOption = "", remarks = "", lastActive = "", dateofJoining = "";
				for (int i = 0; i < dataList.size(); i++) {
					...
					%>
							...
							<tr>
								...
								<td>
									<input type="checkbox" name="cpfno" value="<%=cpfacno%>" onclick="javascript:editPensionMaster('<%=cpfacno%>','<%=employeeName%>','<%=employeeCode%>','<%=region%>','<%=airportCode%>','<%=index%>','<%=totalData%>')" />
								</td>
								<td>
