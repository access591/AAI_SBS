   ...
   <meta http-equiv="keywords" content="keyword1,keyword2,keyword3"/>
	...
	<script type="text/javascript"> 
		...
		function resetSearch(){
    		document.forms[0].action="<%=basePath%>reportservlet?method=penprocessCHQFinApprove";
		...
		function Search(){		
			...
			url ="<%=basePath%>psearch?method=searchForPensionProcess&region="+regionID+"&airPortCode="+airportID+"&empName="+empName+"&dob="+dob+"&doj="+doj+"&employeeNo="+employeeNo+"&pensionno="+pensionno+"&penoption="+penoption+"&empName="+empName+"&frmName=chqFinance";
			//alert(url);
			document.forms[0].action=url;
	  		document.forms[0].method="post";
			document.forms[0].submit();
		}	
		function approveByCHQFin() {
			var value_data=getCheckedValue(document.forms[0].approveradio);	
			if(value_data=="") {
				alert("Please check radio button of the pfid that you want to Approve");
				return false;
			}
	    	var check=new Array();
    		...
    		alert("Cannot edit the data..Already sent to RPFC!!");
    		return false;
    		}
		...
		function deleteRecord() {
	    	...
	    	var verifiedby=check[2];
	    	if( verifiedby=="HRINIT/HRFIN/HRAPPROVE" || verifiedby=="HRINIT/HRFIN/HRAPPROVE/RPFC") {
    		alert("Cannot Delete the data..Already sent to RPFC!!!!");
    		return false;
    		}
    		return false;
		}
   ...
   <form name="personalMaster" action="" method="post">
	 <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
		...
		<%if(!message.equals("")){ %>
				...
				<tr>
				<td  colspan="5"  height="4%"   class="ScreenMasterHeading">CHQ Finance Search 
				</td>
			</tr>
			<tr><td>&nbsp;</td>	</tr>
					<tr>
...
<tr>
		...
		<table  width="70%" border="0" align="center" class="tbborder">			 
				...
				<td align="center" width="50%"><input type="button"  value="Approve" class="btn" onclick="approveByCHQFin();" /></td>
			...
			if (request.getAttribute("searchBean") != null   ) {
				   ...
				   <tr>							
								...
								<display:table  style="width: 750px"   pagesize="15" name="requestScope.searchBean" requestURI="./psearch?method=searchForPensionProcess" >   											
									...
									String status="N";
									if(info.getVerifiedBy().equals("HRINIT/HRFIN")) {
										status="Y";
									}
    								...
    								<input type="radio" name="approveradio" value="<%=info.getPensionNo()%>:<%=info.getProcessID()%>:<%=info.getVerifiedBy()%>" />
    								</display:column>
