...
String frmName="",loginUsrRegion="",message="";
   ...
   <meta http-equiv="keywords" content="keyword1,keyword2,keyword3"/>
	...
	<script type="text/javascript"> 
		...
		function resetSearch(){
	     	document.forms[0].action="<%=basePath%>reportservlet?method=penprocessdatesub";
		...
		function Search(){
			...
			url ="<%=basePath%>psearch?method=searchForPensionProcess&region="+regionID+"&airPortCode="+airportID+"&empName="+empName+"&dob="+dob+"&doj="+doj+"&employeeNo="+employeeNo+"&pensionno="+pensionno+"&penoption="+penoption+"&empName="+empName+"&frmName=RPFCSubmission";
			//alert(url);
		...
		function submitByRPFC() {
			var value_data=getCheckedValue(document.forms[0].submitradio);	
			if(value_data=="") {
				alert("Please check radio button of the pfid that you want to Submit");
				return false;
			}
	    	var check=new Array();
	    	...
	    	if(  verifiedby=="HRINIT/HRFIN/HRAPPROVE/RPFC") {
	    		alert("Cannot edit the data..RPFC has submitted!!");
	    		return false;
	    	}
		...
		function deleteRecords(){ 
	    	...
	    	var process_id=check[1];
	    	var verifiedby=check[2];
	    	if( verifiedby=="HRINIT/HRFIN/HRAPPROVE" || verifiedby=="HRINIT/HRFIN/HRAPPROVE/RPFC") {
	    		alert("Cannot Delete the data..RPFC has submitted!!");
	    		return false;
	    	}
   ...
   <form name="personalMaster" action="" method="post">
	 <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
		...
		<%if(!message.equals("")){%>
				...
				<tr>
				<td  colspan="5"  height="4%"   class="ScreenMasterHeading">Submission Form to RPFC Search 
				</td>
			</tr>
			<tr><td>&nbsp;</td>	</tr>
					<tr>
...
<tr>
		...
		<tr >								 
				<td align="center" width="50%"><input type="button"  value="Submit" class="btn" onclick="submitByRPFC();" /></td>       				
				<td align="left" width="50%"><input type="button"  value="Delete" class="btn" onclick="deleteRecord();" /></td>
			...
			if (request.getAttribute("searchBean") != null   ) {
				   ...
				   <tr>							
								...
								<display:table  style="width: 750px"   pagesize="15" name="requestScope.searchBean" requestURI="./psearch?method=searchForPensionProcess" >   											
									...
									System.out.println("sssssss"+arraylist.get(j)+" "+info.getPensionNo());
								 	String status="N";
									if(info.getVerifiedBy().equals("HRINIT/HRFIN/HRAPPROVE")) {
										status="Y";
									}
								 	%>	
									
    								<display:column   title=""> 
    								<input type="radio" name="submitradio" value="<%=info.getPensionNo()%>:<%=info.getProcessID()%>:<%=info.getVerifiedBy()%>" />
    								</display:column>
