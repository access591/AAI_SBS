  ...
  <head>
   <meta http-equiv="keywords" content="keyword1,keyword2,keyword3"/>
	...
	function resetSearch(){
     				document.forms[0].action="<%=basePath%>reportservlet?method=penprocessCHQHRInitial";
	...
	function Search(){
		...
		url ="<%=basePath%>psearch?method=searchForPensionProcess&region="+regionID+"&airPortCode="+airportID+"&empName="+empName+"&dob="+dob+"&doj="+doj+"&employeeNo="+employeeNo+"&pensionno="+pensionno+"&penoption="+penoption+"&empName="+empName+"&frmName=chqHRInitial";
		//alert(url);
		document.forms[0].action=url;
	  	document.forms[0].method="post";
		document.forms[0].submit();
	}	
	function verifyByCHQHRInitial() {
		var value_data=getCheckedValue(document.forms[0].verifyradio);	
		if(value_data=="") {
			alert("Please check radio button of the pfid that you want to Verify");
			return false;
		}
    	var check=new Array();
    	...
    	if( verifiedby=="HRINIT/HRFIN" || verifiedby=="HRINIT/HRFIN/HRAPPROVE" || verifiedby=="HRINIT/HRFIN/HRAPPROVE/RPFC") {
    		alert("Cannot edit the data..CHQ Finance has already Approved!!");
    		return false;
    	}
//    	var process_id=document.getElementById("process_id").value;
    	//alert(pensioNo+":"+process_id);
		var url="<%=basePath%>psearch?method=editPensionProcess&pensionno="+pensioNo+"&processid="+process_id+"&frmName=chqHRInitial&verifiedby="+verifiedby;	 	
	 	document.forms[0].action=url;
		document.forms[0].method="post";
		document.forms[0].submit();	
	}
	function getCheckedValue(radioObj) {
 	...
 	function deleteRecord() {
    	...
    	var verifiedby=check[2];
    	if( verifiedby=="HRINIT/HRFIN" || verifiedby=="HRINIT/HRFIN/HRAPPROVE" || verifiedby=="HRINIT/HRFIN/HRAPPROVE/RPFC") {
    		alert("Cannot delete the data..CHQ Finance has already Approved!!");
    		return false;
    	}
    	return false;
 	}
   ...
   <form name="personalMaster" action="" method="post">
	 <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
		...
		<%if(!message.equals("")){%>
				...
				<tr>
				<td  colspan="5"  height="4%"   class="ScreenMasterHeading">CHQ HR Approval Search 
				</td>
			</tr>
			<tr><td>&nbsp;</td>	</tr>
					<tr>
...
<tr>
		...
		<table  width="70%" border="0" align="center" class="tbborder">			 
				...
				<td align="center" width="50%"><input type="button"  value="Approve" class="btn" onclick="verifyByCHQHRInitial();" /></td>       				
			...
			if (request.getAttribute("searchBean") != null   ) {
				   ...
				   <tr>							
								...
								<display:table  style="width: 750px"   pagesize="15" name="requestScope.searchBean" requestURI="./psearch?method=searchForPensionProcess" >   											
									...
									String status="N";
									if(info.getVerifiedBy().equals("HRINIT")) {
										status="Y";
									}
    								...
    								<input type="radio" name="verifyradio" value="<%=info.getPensionNo()%>:<%=info.getProcessID()%>:<%=info.getVerifiedBy()%>" />
    								</display:column>
