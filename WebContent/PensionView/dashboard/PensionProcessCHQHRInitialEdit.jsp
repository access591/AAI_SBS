	...
	<script type="text/javascript"> 
		...
		function validate(){
			...
			var procesid=document.getElementById("proces_id").value;   		
   			var chqHrFinDate=document.getElementById("docchqfin").value;   			
   			var chqHrInitialRemarks=document.getElementById("chqhrinitialremarks").value;
		 	if( chqHrFinDate=="" || chqHrFinDate=="---"){
	   			alert("Please Enter Date of case sent to CHQ Finance");
				document.forms[0].docchqfin.focus();
				return false;
   		    }	
   		    if(!chqHrFinDate==""){
	   			var date1=document.forms[0].docchqfin;
		   		var val1=convert_date(date1);
	   	   	    if(val1==false) {
	  		    	return false;
	   		    }
   		    }	
   		    if(chqHrInitialRemarks=="")	{
   		    	//alert("Please enter the remarks");
   		    	//document.forms[0].chqhrinitialremarks.focus();
   		    	//return false;
   		    }
			var url="<%=basePath%>psearch?method=updatepensionprocessCHQHRInitial&chqHRFinDate="+chqHrFinDate+"&chqHrInitialremarks="+chqHrInitialRemarks+"&pensionno="+pensionNo+"&processID="+procesid+"&frmName=chqHRInitial";
			//alert(url);
			//return false;
			document.forms[0].action=url;
	  		document.forms[0].method="post";
...
<tr>
				...
				<td>
					...
					<tr><td class="label">Email Id:</td>
						...
						<tr>
						<td class="label">Date Of Case Sent to CHQ Finance:<font color="red">*</font>
						</td>
						<td><input type="text" name="docchqfin" value="<%=empInfo.getDatetoCHQFIN()%>"  onkeyup="return limitlength(this, 20)">						
						<a href="javascript:show_calendar('forms[0].docchqfin');"><img src="<%=basePath%>PensionView/images/calendar.gif"  alt="Calender" border="no" /></a>
						</td>
						<td class="label">CHQ HR Approval Remarks:</td>
						<td>
					 <TEXTAREA  NAME="chqhrinitialremarks" maxlength="75"   cols="30" tabindex="50"><%=empInfo.getChqHrRemarks()%></TEXTAREA>
						</td>
					</tr>
