  ...
  <head>
	...
	<script type="text/javascript"> 
		...
		function resetParams(pensioNo,process_id,verifiedby){	
			var url="<%=basePath%>psearch?method=editPensionProcess&pensionno="+pensioNo+"&processid="+process_id+"&frmName=RPFCSubmission&verifiedby="+verifiedby; 
			//alert(url);
		...
		function validate(){
			...
			var procesid=document.getElementById("proces_id").value;   		
   			var rpfcDateSubmision=document.getElementById("RpfcSubmisionDate").value;   			
   			var rpfcSubRemarks=document.getElementById("RpfcSubmissionRemarks").value;
		 	if( rpfcDateSubmision=="" || rpfcDateSubmision=="---"){
	   			alert("Please Enter Date of case sent to RPFC Return");
				document.forms[0].RpfcSubmisionDate.focus();
				return false;
   		    }	
   		    if(!rpfcDateSubmision==""){
	   			var date1=document.forms[0].RpfcSubmisionDate;
   		    ...
   		    if(rpfcSubRemarks=="")	{
   		    	...
   		    	//document.forms[0].RpfcSubmissionRemarks.focus();
   		    	//return false;
   		    }
			var url="<%=basePath%>psearch?method=updatepensionprocessRPFCDateofSubmission&rpfcSubmisionDate="+rpfcDateSubmision+"&rpfcSubmisionRemarks="+rpfcSubRemarks+"&pensionno="+pensionNo+"&processID="+procesid+"&frmName=RPFCSubmission";
			//alert(url);		
			//return false;
			document.forms[0].action=url;
	  		document.forms[0].method="post";
...
<tr>
				...
				<td>
					...
					<tr>
						<td class="label">Date Of Case Sent to RPFC :<font color="red">*</font>
						</td>
						<td><input type="text" name="RpfcSubmisionDate" value="<%=empInfo.getRpfcSubmissionDate()%>" onkeyup="return limitlength(this, 20)">		
						<a href="javascript:show_calendar('forms[0].RpfcSubmisionDate');"><img src="<%=basePath%>PensionView/images/calendar.gif"  alt="Calender" border="no" /></a>										
						</td>
						<td class="label">RPFC Submission Remarks:</td>
						<td>
					 <TEXTAREA  NAME="RpfcSubmissionRemarks" maxlength="75"  cols="30" tabindex="50"><%=empInfo.getRpfcSubmisionRemarks()%></TEXTAREA>
						</td>
					</tr>
