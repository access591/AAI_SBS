   ...
   <meta http-equiv="keywords" content="keyword1,keyword2,keyword3"/>
	...
	<script type="text/javascript"> 
		...
		function resetSearch(){
     		document.forms[0].action="<%=basePath%>psearch?method=loadPFIDProcessSearchForm&frmName=PFIDProcessSearch";
		...
		function Search(){
			...
			url ="<%=basePath%>psearch?method=searchForPensionProcess&region="+regionID+"&airPortCode="+airportID+"&empName="+empName+"&dob="+dob+"&doj="+doj+"&employeeNo="+employeeNo+"&pensionno="+pensionno+"&penoption="+penoption+"&empName="+empName+"&frmName=RPFCReturn";
			//alert(url);
		...
		function returnByRPFC() {
			var value_data=getCheckedValue(document.forms[0].returnradio);	
			if(value_data=="") {
   ...
   <form name="personalMaster" action="" method="post">
	 <table width="100%" border="0" align="center" cellpadding="0" cellspacing="0">
		...
		<%if(!message.equals("")){%>
				...
				<tr>
				<td  colspan="5"  height="4%"   class="ScreenMasterHeading">Reject/Return from RPFC search
				</td>
			</tr>
			<tr><td>&nbsp;</td>	</tr>
					<tr>
...
<tr>
		...
		<table  width="70%" border="0" align="center" class="tbborder">			 
				...
				<td align="center" width="50%"><input type="button"  value="Submit" class="btn" onclick="returnByRPFC();" /></td>
				   ...
				   <tr>							
								...
								<display:table  style="width: 750px"   pagesize="15" name="requestScope.searchBean" requestURI="./psearch?method=searchForPensionProcess" >   											
									...
									System.out.println("sssssss"+arraylist.get(j)+" "+info.getPensionNo());
								 	String status="N";
									if(info.getVerifiedBy().equals("HRINIT/HRFIN/HRAPPROVE/RPFC")) {
										status="Y";
									}
								 	%>	
									
    								<display:column   title=""> 
    								<input type="radio" name="returnradio" value="<%=info.getPensionNo()%>:<%=info.getProcessID()%>:<%=info.getVerifiedBy()%>" />
    								</display:column>
