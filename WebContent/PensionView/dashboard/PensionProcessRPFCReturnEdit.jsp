  ...
  <head>
	...
	<script type="text/javascript"> 
		...
		function resetParams(pensioNo,process_id,verifiedby){	
			var url="<%=basePath%>psearch?method=editPensionProcess&pensionno="+pensioNo+"&processid="+process_id+"&frmName=RPFCReturn&verifiedby="+verifiedby; 
			//alert(url);
		...
		function validate(){
			...
			var procesid=document.getElementById("proces_id").value;   		
   			var rpfcDateReturn=document.getElementById("RpfcReturnDate").value;   			
   			var rpfcReturnRemarks=document.getElementById("RpfcReturnRemarks").value;
		 	if( rpfcDateReturn=="" || rpfcDateReturn=="---"){
	   			alert("Please Enter Date of case sent to RPFC Return");
				document.forms[0].RpfcReturnDate.focus();
				return false;
   		    }	
   		    if(!rpfcDateReturn==""){
	   			var date1=document.forms[0].RpfcReturnDate;
		   		var val1=convert_date(date1);
	   	   	    if(val1==false) {
	  		    	return false;
	   		    }
   		    }	
   		    if(rpfcReturnRemarks=="")	{
   		    	//alert("Please enter the remarks");
   		    	//document.forms[0].RpfcReturnRemarks.focus();
   		    	//return false;
   		    }
			var url="<%=basePath%>psearch?method=updatepensionprocessRPFCDateofReturn&rpfcReturnDate="+rpfcDateReturn+"&rpfcReturnRemarks="+rpfcReturnRemarks+"&pensionno="+pensionNo+"&processID="+procesid+"&frmName=RPFCReturn";
			//alert(url);					
			document.forms[0].action=url;
	  		document.forms[0].method="post";
			document.forms[0].submit();		
				...
				<td>
					...
					<tr><td class="label">Email Id:</td>
					 ...
					 <TEXTAREA  NAME="chqhrinitialremarks" maxlength="75" readonly="readonly"  cols="30" tabindex="50"><%=empInfo.getChqHrRemarks()%></TEXTAREA>
						...
						<tr>
						<td class="label">Date Of Case Sent to CHQ HR Approve:
						</td>
						<td><input type="text" name="datechqhrapprove" value="<%=empInfo.getDatetoCHQHR()%>" readonly="readonly" onkeyup="return limitlength(this, 20)">								
						</td>
						<td class="label">CHQ Finance Remarks:</td>
						<td>
					 <TEXTAREA  NAME="chqfinremarks" maxlength="75" readonly="readonly" cols="30" tabindex="50"><%=empInfo.getChqFinRemarks()%></TEXTAREA>
						</td>
					</tr>
					<tr>
						<td class="label">Date Of Case Sent to RPFC:
						</td>
						<td><input type="text" name="RpfcSubmisionDate" value="<%=empInfo.getRpfcSubmissionDate()%>" readonly="readonly" onkeyup="return limitlength(this, 20)">								
						</td>
						<td class="label">RPFC Submission Remarks:</td>
						<td>
					 <TEXTAREA  NAME="RpfcSubmissionRemarks" maxlength="75" readonly="readonly" cols="30" tabindex="50"><%=empInfo.getRpfcSubmisionRemarks()%></TEXTAREA>
						</td>
					</tr>
					<tr>
						<td class="label">Date Of Case Return from RPFC:<font color="red">*</font>
						</td>
						<td><input type="text" name="RpfcReturnDate" value="<%=empInfo.getRpfcReturnDate()%>" onkeyup="return limitlength(this, 20)">								
						<a href="javascript:show_calendar('forms[0].RpfcReturnDate');"><img src="<%=basePath%>PensionView/images/calendar.gif"  alt="Calender" border="no" /></a>										
						</td>
						<td class="label">RPFC Return Remarks:</td>
						<td>
					 <TEXTAREA  NAME="RpfcReturnRemarks" maxlength="75" cols="30" tabindex="50"><%=empInfo.getRpfcReturnRemarks()%></TEXTAREA>
						</td>
					</tr>
