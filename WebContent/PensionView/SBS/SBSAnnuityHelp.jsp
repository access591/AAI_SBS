...
<!-- <link rel="shortcut icon" href="favicon.ico" /> -->
	...
	function testSS(){
  	  ...
  	  if(document.forms[0].empName.value=="" && document.forms[0].pfId.value=="" && document.forms[0].uanNo.value==""){
   		...
   		var url ="<%=basePath%>sbssearch?method=getEmployeeMappedList&region="+region1+"&empName="+empName+"&uanNo="+uanNo+"&transferType="+'<%=transferStatus%>'+"&pfId="+pfId+"&screen="+pcreport+"&adjPfidChkFlag="+'<%=adjPfidChkFlag%>'+"&statemntRevisdChkFlag="+'<%=statemntRevisdChkFlag%>'+"&frmName="+'<%=frmName%>'+"&pcReportChkFlag="+'<%=pcReportChkFlag%>'+"&pagename=annuity";
   		//alert(url);
	 ...
	 function editPensionMaster(airportCode,region,pensionNumber,employeeName,dateofbirth,empSerialNo,dateofseperation,serationreason,doj,designation){
	 var mappedInfo=airportCode+"@"+region+"@"+pensionNumber+"@"+employeeName+"@"+dateofbirth+"@"+empSerialNo;
	...
	if(dateofseperation=="---"){
	 ...
	 window.opener.test(airportCode,serationreason,region,pensionNumber,employeeName,dateofbirth,empSerialNo,dateofseperation,doj,designation);	
	 window.close();	 
	}
	 </script>
	...
	<body>
		...
		<div class="col-md-12" >
			...
			<tr>
                ...
                String dateofBirth = "", empSerialNo = "", dateofJoining = "",designation="";
                ...
                for (int i = 0; i < datalist.size(); i++) {
                    ...
                    %>
						...
						<tr>
							...
							<td>
								<input type="checkbox" name="cpfno" value="<%=cpfacno%>" onclick="javascript:editPensionMaster('<%=airportCode%>','<%=region%>','<%=pensionNumber%>','<%=employeeName%>','<%=dateofBirth%>','<%=empSerialNo%>','<%=dateofSeperation%>','<%=seperationReason%>','<%=dateofJoining %>','<%=designation %>')" />
							</td>
