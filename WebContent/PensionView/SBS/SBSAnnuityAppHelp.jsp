...
<!-- <link rel="shortcut icon" href="favicon.ico" /> -->
	...
	function testSS(){
  	  ...
  	  if(document.forms[0].empName.value=="" && document.forms[0].pfId.value=="" && document.forms[0].appid.value==""){
   		...
   		var url ="<%=basePath%>SBSAnnuityServlet?method=getAnnuityHelp&region="+region1+"&empName="+empName+"&appid="+appid+"&pfId="+pfId;
   		//alert(url);
	 ...
	 function editPensionMaster(formtype,region,gst,empname,corpusamt,empSerialNo,appid){
	 //alert(appid);
	...
	//document.forms[0].submit();
	//alert(corpusamt);
	 window.opener.test(formtype,region,gst,empname,corpusamt,empSerialNo,appid);	
	 window.close();	 
	}
	 </script>
                ...
                for (int i = 0; i < datalist.size(); i++) {
                    ...
                    %>
						...
						<tr>
							...
							<td>
								<input type="checkbox" name="cpfno" value="<%=formtype%>" onclick="javascript:editPensionMaster('<%=formtype%>','<%=region%>','<%=gst%>','<%=employeeName%>','<%=corpusamt%>','<%=empSerialNo%>','<%=appid %>')" />
							</td>
