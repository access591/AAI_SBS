...
<html>
	...
	<body>
<table align="center"   cellpadding="0px" cellspacing="0px"><tr height="25px" style="background:#378fe7;color:#ffffff"><td align="center">HDFC Life Proposal Form</td></tr>

<tr><td>
		<table width="100%" border="1" cellpadding="3px" cellspacing="0">
		
			<tr>
		<td width="50%">Primary Annuitant's Name(As per Bank Records)</td><td><%=bean.getMemberName()%></td>
			</tr>
		...
		<td>Date of Birth</td><td><%=bean.getDob() %></td>
			...
			<tr>
		<td>Employee Number</td><td><%=bean.getEmployeeNo()%></td>
			</tr>
			<tr>
		<td>Gender of primary Annuitant</td><td><%=bean.getGender() %></td>
			</tr>
			<tr>
		<td> Address with Pin Code</td><td><%=bean.getMemberPerAdd() %></td>
			</tr>
			<tr>
		<td>Mobile Number</td><td><%=bean.getMobilNo() %></td>
			</tr>
			<tr>
		<td>Alternate contact Number</td><td><%=bean.getMobilNo1() %></td>
			</tr>
			<tr>
		...
		<td>IFSC Code</td><td><%=bean.getIfscCode() %></td>
			...
			<tr>
		<td>MICR Code</td><td><%=bean.getMicrCode() %></td>
			</tr>
		...
		<td width="50%">Corpus Amount/Purchase Price(In Rs)</td><td><%=bean.getCorpusAmt() %></td>
			...
			<tr>
		<td>Name of the Annuity Service Provider</td><td>HDFC Life Insurance Company Ltd</td>
			</tr>
		...
		<td>Joint Life Annuity<%=bean.getAaiEDCPSoption().equals("C")?"<img width:12px; height:15px; src='assets/img/check_mark.png'/>":"" %></td><td>Joint Life Annuity with Return Of Purchase Price<%=bean.getAaiEDCPSoption().equals("D")?"<img width:12px; height:15px; src='assets/img/check_mark.png'/>":"" %></td>
			...
			<table width="100%" border="1" cellpadding="3px" cellspacing="0">
		<tr><th colspan="2">In Case the Option chosen is 'joint Life',please fill the below details:</th></tr>
			<tr>
		<td width="50%">Name of the secondary annuitant</td><td><%=bean.getSpouseName() %></td>
			</tr>
			<tr>
		<td width="50%">Gender of secondary annuitant</td><td><%=bean.getSecAnnuitantGender()%></td>
			</tr>
			<tr>
		...
		<tr><th colspan="6">Nominee Details </th></tr>
			<tr>
		<th>S.no</th><th>Nominee Name</th><th>Nominee Date of Birth</th><th>Nominee Relation With Primary Annuitant</th><th>Distribution Percentage</th><th>Nominee Gender</th>
			</tr>
			...
			<tr><td><%=i+1 %></td> <td><%=nomineeBean.getNomineename() %></td><td><%=nomineeBean.getNomineeDOB() %></td><td><%=nomineeBean.getNomineeRelation() %></td><td><%=nomineeBean.getPercentage() %></td><td><%=nomineeBean.getGender()%></td></tr>
		...
		<th>S.no</th><th>Nominee Name</th><th>Name of the appointee</th><th>Appointee Date of Birth</th><th>Appointee Relationship with Nominee</th><th>Appointee's Mobile Number</th><th>Appointee's Adress</th>
			...
			<tr><td><%=i+1 %></td> <td><%=appointeeBean.getNomineename() %></td><td><%=appointeeBean.getAppointeeName() %></td><td><%=appointeeBean.getNomineeDOB() %></td><td><%=appointeeBean.getNomineeRelation() %></td><td><%=appointeeBean.getAppointeeMobile() %></td><td><%=appointeeBean.getAppointeeAddress() %></td></tr>
			...
			<table width="100%" border="1" cellpadding="7px" cellspacing="0">
