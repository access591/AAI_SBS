...
<%
  ...
  <tr>
    <td colspan="7"><table width="100%" border="1" cellspacing="0" cellpadding="0">
      ...
      <tr>
        ...
        <td rowspan="2" class="label">Edit</td>
      </tr>
      <tr>
      ...
      <tr>
        ...
        <%
       	...
       	double totalAAIPF=0.0,totalPFDrwn=0.0,totalAAINet=0.0,advancePFWPaid=0.0,empNetInterest=0.0,aaiNetInterest=0.0,totalPensionContr=0.0,pensionInterest=0.0;
		...
		Long cpfAdjOB=null,pensionAdjOB=null,pfAdjOB=null,empSubOB=null;
	    ...
	    System.out.println("MonthYear"+pensionCardInfo.getShnMnthYear()+"totalEmpNet"+totalEmpNet);
	    ...
	    totalPFDrwn= new Double(df.format(totalPFDrwn+Math.round( Double.parseDouble(pensionCardInfo.getPfDrawn())))).doubleValue();
	      ...
	      System.out.println("MonthYear"+pensionCardInfo.getShnMnthYear()+"totalAAINet"+totalAAINet);
	 ...
	 <td width="4%" class="NumData"><input type="text"  size="5" name="emoluments" value="<%=Math.round(Double.parseDouble(pensionCardInfo.getEmoluments()))%>"/></td>
	 <td width="5%" class="NumData"><%=Math.round(Double.parseDouble(pensionCardInfo.getEmppfstatury()))%></td>
	 <td width="3%" class="NumData"><input type="text" size="4" name="vpf" value="<%=Math.round(Double.parseDouble(pensionCardInfo.getEmpvpf()))%>"/></td>
	 <td width="5%" nowrap="nowrap" class="NumData"><input type="text" size="5" name="principle" value="<%=Math.round(Double.parseDouble(pensionCardInfo.getPrincipal()))%>"/></td>
	 <td width="5%" class="NumData"><input type="text"  size="5" name="interest" value="<%=Math.round(Double.parseDouble(pensionCardInfo.getInterest()))%>"/></td>
	 <td width="5%" class="NumData"><%=Math.round(Double.parseDouble(pensionCardInfo.getEmptotal()))%></td>
	 ...
	 <td width="2%" class="Data"><input type="button" name="edit" value="E"/></td>
	 <input type="hidden" name="monthInfo" value="'<%=pensionCardInfo.getShnMnthYear()%>','<%=Math.round(Double.parseDouble(pensionCardInfo.getEmoluments()))%>','<%=Math.round(Double.parseDouble(pensionCardInfo.getEmppfstatury()))%>','<%=Math.round(Double.parseDouble(pensionCardInfo.getEmpvpf()))%>','<%=Math.round(Double.parseDouble(pensionCardInfo.getPrincipal()))%>','<%=Math.round(Double.parseDouble(pensionCardInfo.getInterest()))%>','<%=personalInfo.getPfID()%>','<%=personalInfo.getWetherOption().toString().trim()%>'"/>
	 ...
	 <%
	 	...
	 	if(pensionCardInfo.getCbFlag().equals("Y")){
	 		...
	 		System.out.println("Size of Opening Balances"+obList.size()+"totalEmpNet"+totalEmpNet+"totalAAINet"+totalAAINet);
	 		if(obList.size()!=0){
	 				if(Integer.parseInt(calYear)>=1995 && Integer.parseInt(calYear)<2000){
	 					...
	 					rateOfInterest=9.50;
	 				}else if(Integer.parseInt(calYear)>=2005 && Integer.parseInt(calYear)<2009){
	 					rateOfInterest=8.50;
	 				}else if(Integer.parseInt(calYear)>=2009 && Integer.parseInt(calYear)<2010){
	 					rateOfInterest=8.00;
	 				}
				    ...
				    System.out.println("totalEmpNet"+totalEmpNet+"totalNoOfMonths"+noOfmonthsForInterest+"empNetInterest"+empNetInterest+"aaiNetInterest"+aaiNetInterest);
				 	closingOBList=reportService.calClosingOB(rateOfInterest,obList,Math.round(dispTotalAAINet),Math.round(aaiNetInterest),Math.round(dispTotalEmpNet),Math.round(empNetInterest),Math.round(totalPensionContr),Math.round(pensionInterest),advancePFWPaid,principle,noOfmonthsForInterest);
 ...
 <tr>
	 ...
	 <td width="12%" class="NumData">0</td>
 ...
 %>
  <tr>
   ...
   <tr>
	 <td colspan="2" nowrap="nowrap" class="label">NET CLOSING</td>
 	 <td colspan="6" class="Data">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</td>
 ...
 <tr>
  ...
  <tr>
    ...
    <td colspan="15" align="center"><input type="button" class="btn"  value="Submit" onclick="updateMonthlyCpf()"/></td>
