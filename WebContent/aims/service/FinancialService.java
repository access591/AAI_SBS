...
import aims.bean.CompartiveReportBean;
...
public class FinancialService {
	...
	public SearchInfo getFinancialData(String monthYear, String unitCode,
			int gridLength) {
		...
		try {
			finSelectedDt = common.converDBToAppFormat(monthYear, "yyyyMM",
					"MMM-yy");
		...
		financialList = financialDAO.financialDataAll(finSelectedDt,
				startIndex, unitCode);
		...
		log.info("============startIndex(getFinancialData)==========="
				+ startIndex);
	...
	public EmployeeValidateInfo getFinancialLoadEditDetails(String cpfaccno,
			String unitCode, String effectiveDate) {
		String finSelectedDt = "";
		EmployeeValidateInfo financialLodBean = new EmployeeValidateInfo();

		financialLodBean = financialDAO.getPensionValidateDetails(cpfaccno,
				unitCode, effectiveDate);
		return financialLodBean;

	}

	public int updateFinancialDetails(String cpfaccno, String unitCode,
			String valPF, String valPension, String valTotal, String pfStatury,
			String pfDedcution, String userName, String effectiveDate) {
		int updateVal = 0;

		updateVal = financialDAO.updatePensionValidateDetails(cpfaccno,
				unitCode, valPF, valPension, valTotal, pfStatury, pfDedcution,
	...
	public SearchInfo navigationFinancialData(String monthYear,
			String unitCode, SearchInfo searchInfo, int gridLength) {
		int startIndex = 0;
		...
		startIndex = finpaging.getPageIndex(navButton, startIndex, rowCount,
				gridLength);
		log.info("rowCount=========navigationFinancialData=========" + rowCount
				+ "startIndex" + startIndex + "navButton" + navButton);
		hindiData = financialDAO.financialDataAll(finSelectedDt, startIndex,
				unitCode);
	...
	public EmployeeValidateInfo getLoadFinancialEditDetails(String cpfaccno,
		...
		validateBean = financialDAO.getFinanceEditDetails(cpfaccno, pensionno,
				unitCD, effectiveDate, employeeNo, designation, employeeName,
				region);
		return validateBean;
	}

	public int financialUpdateDetails(EmployeeValidateInfo validateUpdateInfo,
			String airportCD, String effectiveDate, String pensionno,
			String cpfacno, String employeeName, String userID,
			String computerName, String region) {
		int count = 0;
		EmployeeValidateInfo validateBean = new EmployeeValidateInfo();
		count = financialDAO.updateFinanceDetail(validateUpdateInfo, airportCD,
				effectiveDate, pensionno, cpfacno, employeeName, userID,
				computerName, region);
		return count;
	}

	public ArrayList getFinanceListAll(String fromDate, String toDate)
			throws InvalidDataException {
		ArrayList list = new ArrayList();
		fromDate = common.converDBToAppFormat(fromDate, "dd/MM/yyyy",
				"dd-MMM-yyyy");

		toDate = common
				.converDBToAppFormat(toDate, "dd/MM/yyyy", "dd-MMM-yyyy");
		log.info("getFinanceListAll==================" + fromDate + "toDate"
				+ toDate);
	...
	public ArrayList financeReportByEachEmp(String cpfaccno, String employeeno) {
		...
		financialrptList = financialDAO.financeReportByEachEmp(cpfaccno,
				employeeno);
	...
	public ArrayList financeValidateReport(String monthYear, String unitCode) {
		...
		financialrptList = financialDAO.financeValidateReport(finSelectedDt,
				unitCode);
	...
	public ArrayList financeReportMissingMonthsByEachEmp(String cpfaccno,
		...
		log
				.info("financeReportMissingMonthsByEachEmp=======Service====employeeNo========"
						+ employeeno);
		financialMissingList = financialDAO
				.financeReportMissingMonthsByEachEmp(cpfaccno, employeeno,
						region);
		return financialMissingList;
	}

	public SearchInfo getFinancialReport(String region, String flag,
			int gridLength) {
		int startIndex = 0, count = 0;
		ArrayList financialPersonalList = new ArrayList();
		SearchInfo searchInfo = new SearchInfo();
		startIndex = 1;
		Pagenation finsearpaging = new Pagenation();
		financialPersonalList = financialDAO.getFinancalInfo(region, flag,
				startIndex, gridLength);
		...
		bottomGridNavigationInfo = finsearpaging.searchPagination(count,
				startIndex, gridLength);
		log.info("============startIndex(getFinancalReport)==========="
				+ startIndex);
		searchInfo.setStartIndex(startIndex);
	...
	public SearchInfo getnavigationFinancialReport(String region, String flag,
			int gridLength, SearchInfo searchInfo) {
		int startIndex = 0;
		String navButton = "", finSelectedDt = "";
		...
		startIndex = finpaging.getPageIndex(navButton, startIndex, rowCount,
				gridLength);
		log.info("rowCount=========getnavigationFinancialReport========="
				+ rowCount + "startIndex" + startIndex + "navButton"
				+ navButton);
		financialReportList = financialDAO.getFinancalInfo(region, flag,
				startIndex, gridLength);
		bottomGridNavigationInfo = finpaging.navigationPagination(rowCount,
				startIndex, false, false, gridLength);
		searchInfo.setStartIndex(startIndex);
	...
	public ArrayList financeForm3Report(String airportCode, String date,
		...
		form3tList = financialDAO.financeForm3Report(airportCode, date, region,
				retriedDate, sortingOrder, empName, empNameFlag);
		return form3tList;
	}

	public ArrayList financeDataAvailableCheck(String airportCode, String date,
			String region, String retriedDate, String sortingOrder) {
		ArrayList form3tList = new ArrayList();
		// form3tList=financialDAO.financeForm3Report(airportCode,date,region,
		// retriedDate,sortingOrder);
		form3tList = financialDAO.financeForm3Report(airportCode, date, region,
				retriedDate, sortingOrder, "", "");
		return form3tList;
	}
	...
	public int[] financeForm6BCountData(String airportCode, String date,
		...
		return financialDAO.financeForm6BCountData(airportCode, date, region,
				prevDate);
	}
	...
	/*
	 * public ArrayList financeForm45Report(String airportCode,String
	 * date1,String date2,String region){ ArrayList form45tList=new ArrayList();
	 * form45tList
	 * =financialDAO.financeForm45Report(airportCode,date1,date2,region); return
	 * form45tList; }
	 */
	...
	public CompartiveReportBean getPensionValidateData(String region) {
		...
		log.info(".........getPensionValidateData in service ..........."
				+ pensionList.size());
		log.info(".........getAAIPensionData in service ..........."
				+ AAIList.size());
			...
			if (pensionList.size() > AAIList.size()) {
				...
				for (int i = 0; i < pensionList.size(); i++) {
					for (int j = 0; j < AAIList.size(); j++) {
						...
						compartiveReportform3info = (CompartiveReportForm3DataBean) pensionList
								.get(i);
						compartiveReportAaiinfo = (CompartiveReportAaiDataBean) AAIList
								.get(j);
						if ((compartiveReportform3info.getForm3CPFaccno())
								.equals(compartiveReportAaiinfo
										.getAaiCPFaccno())) {
				...
				log.info(".............commonList size.................."
						+ commonList.size());
				log.info(".............AAIList size.................."
						+ AAIList.size());
				log.info(".............temppensionList size.................."
						+ temppensionList.size());
				...
				log.info(".............commonList size.................."
						+ commonList.size());
				log
						.info(".............commonPensionList size.................."
								+ commonPensionList.size());
				log.info(".............pensionList size.................."
						+ pensionList.size());
				log.info(".............tempAAIList size.................."
						+ tempAAIList.size());
	...
	public ArrayList financeForm4Report(String airportCode, String date1,
			String date2, String region, String sortingOrder, String reportType) {
		ArrayList form45tList = new ArrayList();
		form45tList = financialDAO.financeForm4ReportByDJ(airportCode, date1,
				date2, region);
		// form45tList=financialDAO.financeForm45Report(airportCode,date1,date2,
		// region,sortingOrder,reportType);

		return form45tList;
	}

	public ArrayList financeForm5Report(String airportCode, String date1,
			String date2, String region, String sortingOrder, String reportType) {
		ArrayList form45tList = new ArrayList();

		//form45tList=financialDAO.financeForm5ReportByDJ(airportCode,date1,date2
		// ,region);
		form45tList = financialDAO.financeForm45Report(airportCode, date1,
				date2, region, sortingOrder, reportType);
	...
	public ArrayList financeForm6AReport(String airportCode, String date,
		...
		form6AList = financialDAO.financeForm6AReport(airportCode, date,
				region, sortingOrder);
		return form6AList;
	}
	...
	public ArrayList financeForm6BReport(String airportCode, String date,
			String region, String sortingOrder) {
		ArrayList form6BList = new ArrayList();
		form6BList = financialDAO.financeForm6BReport(airportCode, date,
				region, sortingOrder);
		return form6BList;
	}
	...
	public ArrayList financeForm6Report(String airportCode, String date,
		...
		form6List = financialDAO.financeForm6Report(airportCode, date, region,
				sortingOrder);
		return form6List;
	}
	...
	public ArrayList financeForm3ReportAll(String date, String region,
			...
			String empNameFlag) {
		ArrayList form3tList = new ArrayList();
		form3tList = financialDAO.financeForm3ReportAll(date, region,
				retriedDate, sortingOrder, empName, empNameFlag);
		return form3tList;
	}
	...
	public int[] financeRpfcFormsCountData(String airportCode, String date,
			String region, String prevDate) {
		return financialDAO.financeRpfcFormsCountData(airportCode, date,
				region, prevDate);
	}
	...
	public int financePfidUpdate(String cpfaccno, String employeeCode,
		...
		int count = financialDAO.financePfidUpdate(cpfaccno, employeeCode,
				region, pfid);
	...
	public ArrayList deviationReport(String pfidString, String region,
			String airportcode, String frmSelectedDts, String flag,
			String employeeName, String sortedColumn, String pensionNo) {
		ArrayList AAIEPFLst = new ArrayList();
		AAIEPFLst = financialDAO.deviationReport(pfidString, region,
				airportcode, frmSelectedDts, flag, employeeName, sortedColumn,
				pensionNo);
		return AAIEPFLst;
	}
	...
	public ArrayList searchYearlyEmpCount(FinacialDataBean databean)
			throws Exception {
	...
	public ArrayList transferReport(String region, String airportcode,
		...
		ArrayList AAIEPFLst = new ArrayList();
		AAIEPFLst = financialDAO.transferReport(region, airportcode,
				frmSelectedDts, sortedColumn, pensionNo);
		return AAIEPFLst;
	}
	...
	public void updateTransferData(String pensionno, String seperationreason,
		...
		financialDAO.updateTransferData(pensionno, seperationreason,
				sepetaionDt, airportcode, region, fromairportcode, fromregion,
				status);

	}

	public ArrayList rpfcForm5Report(String date1, String date2, String region,
			String pensionNo, String airportCode, String sortingOrder) {
		ArrayList form45tList = new ArrayList();

		//form45tList=financialDAO.financeForm5ReportByDJ(airportCode,date1,date2
		// ,region);
		form45tList = financialDAO.rpfcForm5ReportByDJ(date1, date2, region,
				pensionNo, airportCode, sortingOrder);

		log.info("financeForm5Report LIst size" + form45tList.size());
		return form45tList;
	}

	public ArrayList rpfcForm4Report(String airportCode, String date1,
			String date2, String region, String pensionno, String sortingOrder) {
		ArrayList form4tList = new ArrayList();
		form4tList = financialDAO.rpfcForm4ReportByDJ(airportCode, date1,
				date2, region, pensionno, sortingOrder);
		return form4tList;
	}
	...
	public ArrayList saveCpfAdjustMents(String emoluments, String cpf,
			...
			String cpfacno) {
		ArrayList a = null;
		a = finReportDAO.saveCpfAdjustMents(emoluments, cpf, vpf, principle,
				interest, pensionno, month, year, pccontrib, transmonthyear,
				computername, userName, region, advAmount, loan_sub_amt,
				loan_cont_amt, cpfacno);
		return a;
	}
	...
	public ArrayList getArrearsData(String pensionno, int strtIndex,
		...
		empList = financialDAO.getArrearsData(pensionno, strtIndex, gridLength,
				"", "");
		return empList;
	}
	...
	public ArrayList getArrearsTransactionData(String pensionno,
		...
		transactionData = financialDAO.getArrearsTransactionData(pensionno,
				dateOfAnnuation, flag, employeename);
	...
	public SearchInfo navigationArrearsData(String navgationBtn,
			String pensionno, int strtIndex, int gridLength, String empName,
			String pensioncontriadj) throws Exception {
		log.info("===========Enter navigationArrearsData====================");
		...
		int totalRecords = financialDAO.totalCountArrears(pensionno, empName);
		BottomGridNavigationInfo bottomGridNavigationInfo = new BottomGridNavigationInfo();
		// bottomGridNavigationInfo = paging.searchPagination(totalRecords,
		// strtIndex, gridLength);

		startIndex = paging.getPageIndex(navgationBtn, startIndex,
				totalRecords, gridLength);
		if (navgationBtn.trim().equals("|<") && pensionno.trim().equals("")) {
			onlyFirst = false;
			onlyLast = true;
		} else if (navgationBtn.trim().equals(">|")
		...
		bottomGridNavigationInfo = paging.navigationPagination(totalRecords,
				strtIndex, onlyFirst, onlyLast, gridLength);
		empList = financialDAO.getArrearsData(pensionno, startIndex,
				gridLength, empName, pensioncontriadj);

		log.info("emp list size " + empList.size());

		SearchInfo searchInfo = new SearchInfo();
		searchInfo.setStartIndex(startIndex);
		searchInfo.setSearchList(empList);
		// searchInfo.setSearchList(empInfo);
		searchInfo.setTotalRecords(totalRecords);
		searchInfo.setBottomGrid(bottomGridNavigationInfo);
		log.info("=============navigationArrearsData====================");
		return searchInfo;

	}
	public void movedatafrom95to2008(String PFID,String computerName,String userName) {
		financialDAO.movedatafrom95to2008(PFID,computerName,userName);
	}
	public double getPensionContributionTotal(String PFID){
		return financialDAO.getPensionContributionTotal(PFID);
	}
	public double getfinalrevoveryintrest(String PFID){
		return financialDAO.getfinalrevoveryintrest(PFID);
	}
	public String getEditedPensionno(String PFID){
		return financialDAO.getEditedPensionno(PFID);
	}
	public void optionChange(String pfid,String optionChange,String dateofbirth,String empname, String userName,
			String password) {
		financialDAO.optionChange(pfid, optionChange,dateofbirth,empname,userName, password);
	}
	public void saveFinalRecoveryIntrest(String intrest,String pensionno,String employeename,String desig,String station,String region,String interestCalcDt,String username,String ComputerName,String difftotals,String revisedpc,String difcontri) {
		System.out.println("finasialservice::saveFinalRecoveryIntrest method");
		financialDAO.saveFinalRecoveryIntrest(intrest,pensionno,employeename,desig,station,region,interestCalcDt,username,ComputerName,difftotals,revisedpc,difcontri);
	}	
	public SearchInfo financeDataReport(EmpMasterBean empSearch,
			SearchInfo searchInfo, int gridLength, String empNameCheck,
			String reporttype, String pfidfrom) {
		log.info("FinancialService:financeDataReport Entering method"+reporttype);
		ArrayList financeDataList = new ArrayList();
		int startIndex = 1;
		...
		financeDataList = financialDAO.financeDataReport(empSearch, gridLength,
				startIndex, empNameCheck, unmappedFlag, reporttype,
				allRecordsFlag, pfidfrom);
		searchInfo.setSearchList(financeDataList);
		return searchInfo;
	}
	public SearchInfo adjCrtnDataSearch(EmpMasterBean empSearch,SearchInfo searchInfo,String empNameCheck) {
		
		ArrayList financeDataList = new ArrayList();
		int totalRecords = financialDAO.adjCrtnTotalCountPersonal(empSearch,empNameCheck);
		financeDataList = financialDAO.adjCrtnDataSearch(empSearch,empNameCheck);
		searchInfo.setSearchList(financeDataList);
		searchInfo.setTotalRecords(totalRecords);
		return searchInfo;
	}
	...
	public void adjMappingUpdate(String employeeCode,String cpfaccno,String employeeName,String region,String pensionno,String designation,String dateOfBirth,String dateOfJoining,String pensionOption){
		financialDAO.adjMappingUpdate(employeeCode,cpfaccno,employeeName,region,pensionno,designation,dateOfBirth,dateOfJoining,pensionOption);
	}
}
